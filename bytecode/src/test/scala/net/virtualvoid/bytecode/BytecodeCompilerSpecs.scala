...
object BytecodeCompilerSpecs extends Specification {
  def safe[R](x: => R): R = try { x } catch { case e: Throwable => e.printStackTrace; null.asInstanceOf[R] }
  def compiledTests(compiler:net.virtualvoid.bytecode.Bytecode.ByteletCompiler) { try {
    import Bytecode._
    import Bytecode.Implicits._
    import Bytecode.Instructions._
    
    val boxInt: Method1[Int,java.lang.Integer] = safe(method1(Integer.valueOf(_:Int)))
