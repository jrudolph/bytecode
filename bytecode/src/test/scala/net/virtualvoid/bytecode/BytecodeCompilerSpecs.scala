...
object BytecodeCompilerSpecs extends Specification{
  def compiledTests(compiler:net.virtualvoid.bytecode.Bytecode.ByteletCompiler){
    ...
    "locals + method2" in {
      compiler.compile(classOf[java.lang.String])(_ ~ local(_0).store() ~ local(_0).load()~ local(_0).load() ~ invokemethod2(_.concat(_)))
    ...
    "store(_) int in locals" in {
      compiler.compile(classOf[java.lang.Integer])(_~invokemethod1(_.intValue)~dup~local(_0).store()~local(_0).load()~iadd~invokemethod1(Integer.valueOf(_)))
      .apply(12) must be_==(24)}
    "store(_) double in locals" in {
      compiler.compile(classOf[java.lang.Double])(_~invokemethod1(_.doubleValue)~local(_0).store()~local(_0).load()~invokemethod1(java.lang.Double.valueOf(_)))
      .apply(12.453) must be_==(12.453)}
    "store(_) double after method2" in {
      compiler.compile(classOf[java.lang.Double])(_~invokemethod1(_.doubleValue)~ldc("test")~dup~invokemethod2(_.concat(_))~pop~local(_0).store() ~ local(_0).load()~invokemethod1(java.lang.Double.valueOf(_:Double)))
      .apply(12.453) must be_==(12.453)}
    "store(_) something more than 1 level deep" in {
      compiler.compile(classOf[String])(_~local(_1).store() ~ local(_1).load())
    ...
    "store(_) string after void method" in {
      compiler.compile(classOf[java.lang.String])(_ ~ newInstance(classOf[java.text.SimpleDateFormat]) ~ ldc("yyyy") ~ invokemethod2(_.applyPattern(_)) ~ pop_unit ~ local(_0).store() ~ local(_0).load())
      .apply("test") must be_==("test")
    }
      ...
      compiler.compile(classOf[java.lang.Integer])(
        ...
        f => {
          val start = f ~
            ...
            local(_0).store() ~ //  store(_) current i in local 0
            bipush(0) ~
            local(_1).store() ~ //  store(_) sum in local 1
            target
          
          start ~
            local(_0).load() ~ // load i to check if it is already 0 
            ifne(f => 
              f ~ 
                local(_0).load() ~
                dup ~
