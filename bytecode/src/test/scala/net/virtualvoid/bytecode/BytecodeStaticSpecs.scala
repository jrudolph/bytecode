...
"""+prefix+str+suffix+"}")
  ...
  "implicits" should apply {
    ...
    "l.load.e.dup.iadd with Int local" in {Locals("Nil**Int") must haveOp("(local[_0,Int].load(_))~dup~iadd")}
    "l.l.load.e.e.dup.iadd with Int local on place 2" in {Locals("Nil**Int**String") must haveOp("(local[_1,Int].load(_))~dup~iadd")}
    "l.store.e on no locals (should generate one local)" in {Frame("Nil**String","Nil") must haveOp("(local[_0,String].store(_))~(local[_0,String].load(_))~method(_.length)")}
