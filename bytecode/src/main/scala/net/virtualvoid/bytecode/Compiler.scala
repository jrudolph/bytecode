...
object ASMCompiler extends ByteletCompiler{
    ...
    class UnsetClassStack(lrest:ClassStack) extends ClassStack(lrest,null){
      ...
      override def set(i:Int,cl:Class[_]):ClassStack = 
        if (i>0) new UnsetClassStack(set(i-1,cl)) else ClassStack(this,cl)  
    }
    ...
    class ASMFrame[ST<:List,LT<:List](mv:MethodVisitor
      ...
      case class ASMBackwardTarget[ST<:List,LT<:List](mv:MethodVisitor
                                                      ,stackClass:ClassStack
                                                      ,localsClass:ClassStack
                                                      ,label:Label)
          extends ASMFrame[ST,LT](mv,stackClass,localsClass) 
          	with BackwardTarget[ST,LT] with ASMTarget
      ...
      case class ASMForwardTarget[ST<:List,LT<:List](label:Label) 
      	extends ForwardTarget[ST,LT] with ASMTarget
      ...
      def swap_int[R<:List,T1,T2](rest:R,t2:T2,t1:T1):F[R**T1**T2,LT] = {
        ...
        new ASMFrame[R**T1**T2,LT](mv
        	,stackClass.rest.rest**stackClass.top**stackClass.rest.top,localsClass)
      }
      def dup_x1_int[R<:List,T1,T2](rest:R,t2:T2,t1:T1):F[R**T1**T2**T1,LT] = {
        mv.visitInsn(DUP_X1)
        new ASMFrame[R**T1**T2**T1,LT](mv
          ,stackClass.rest.rest**stackClass.top**
            stackClass.rest.top**stackClass.top,localsClass)
      }
      ...
      def ifne2_int[R<:List,ST2<:List,LT2<:List]
      (rest:R,top:JVMInt,then:F[R,LT]=>F[ST2,LT2],elseB:F[R,LT]=>F[ST2,LT2])
      :F[ST2,LT2] = {
        /*
         * ifeq thenLabel
         *   elseB
      ...
      def invalidFrame[ST<:List,LT<:List]:F[ST,LT] = 
    ...
    (code: Return[U] => F[Nil**T,Nil]=>F[Nil**U,_]): T => U = {
      i+=1
      ...
      { // apply
        ...
        code(new Return[U]{
        	def jmp[LT<:List]:F[Nil**U,LT] => Nothing = f => { 
        		mv.visitInsn(ARETURN)
        		throw JmpException
        	}
          }     
        )(new ASMFrame[Nil**T,Nil](mv,EmptyClassStack ** cl,EmptyClassStack))
