...
trait Func[Args <: Stack, U] {
   ...
   * Stack which has the parameters on top.
   */
  def apply[R <: Stack, Res <: Stack](implicit sub: Sub[R, Args, Res]): R => Res**U =
    st => sub.res(st) match {
      case (b, r) => r ** apply(b)
    }
}
...
trait SubstractionImplicits {
   ...
   * we can return R.
   */
  implicit def sub0[R <: Stack]: Sub[R, Nil, R] = new Sub[R, Nil, R] {
    def res(a: R) = (N, a)
  }
  /**
   ...
   * implicit parameter.
   */
  implicit def subt[R1 <: Stack, T, R2 <: Stack, Res <: Stack](implicit s: Sub[R1, R2, Res]): Sub[R1**T, R2**T, Res] =
...
trait ListShortcuts extends SubstractionImplicits {
  ...
  type |=>[R <: Stack, U] = Func[R, U]

  trait Consable[T <: Stack] {
    def **[U](next: U): T**U
  }

  // make every type consable into a heterogenous list 
  implicit def conser[T <: Stack](t: T) = new Consable[T] {
